...
import net.draimcido.draimfarming.api.event.CustomWorldEvent;
...
public class CustomWorld {
    ...
    private void backUp(String worldName) {
        ...
        File file = new File(Main.plugin.getDataFolder().getParentFile().getParentFile(), worldName + File.separator + "draimfarming_data");
        File[] files = file.listFiles();
        if (files == null) return;
        for (File data : files) {
            FileUtils.copyFileToDirectory(data, new File(Main.plugin.getDataFolder(), "backup" + File.separator + worldName + "_" + format.format(date)));
        }
    }
    ...
    private void loadScarecrow() {
        ...
        try {
            ...
            JsonElement json= jsonParser.parse(new FileReader(new File(Main.plugin.getDataFolder().getParentFile().getParentFile(), world.getName() + File.separator + "draimfarming_data" + File.separator + "scarecrow.json")));
            ...
            SeasonUtils.setSeason(world, DFSeason.UNKNOWN);
        }
    ...
    private void unloadScarecrow() {
        ...
        try (FileWriter fileWriter = new FileWriter(new File(Main.plugin.getDataFolder().getParentFile().getParentFile(), world.getName() + File.separator + "draimfarming_data" + File.separator + "scarecrow.json"))){
    ...
    private void loadSeason() {
        ...
        try {
            JsonParser jsonParser = new JsonParser();
            JsonElement json= jsonParser.parse(new FileReader(new File(Main.plugin.getDataFolder().getParentFile().getParentFile(), world.getName() + File.separator + "draimfarming_data" + File.separator + "season.json")));
            if (json.isJsonObject()) {
                JsonObject jsonObject = json.getAsJsonObject();
                if (jsonObject.has("season")) {
                    ...
                    SeasonUtils.setSeason(world, DFSeason.valueOf(season));
                    return;
                }
            }
            SeasonUtils.setSeason(world, DFSeason.UNKNOWN);
    ...
    private void unloadSeason() {
        ...
        try (FileWriter fileWriter = new FileWriter(new File(Main.plugin.getDataFolder().getParentFile().getParentFile(), world.getName() + File.separator + "draimfarming_data" + File.separator + "season.json"))){
            fileWriter.write(jsonObject.toString().replace("\\\\", "\\"));
        } catch (IOException e) {
            e.printStackTrace();
        }
    ...
    private void loadPot() {
        try {
            JsonParser jsonParser = new JsonParser();
            JsonElement json= jsonParser.parse(new FileReader(new File(Main.plugin.getDataFolder().getParentFile().getParentFile(), world.getName() + File.separator + "draimfarming_data" + File.separator + "pot.json")));
            if (json.isJsonObject()) {
                JsonObject jsonObject = json.getAsJsonObject();
    ...
    private void unloadPot() {
        ...
        try (FileWriter fileWriter = new FileWriter(new File(Main.plugin.getDataFolder().getParentFile().getParentFile(), world.getName() + File.separator + "draimfarming_data" + File.separator + "pot.json"))){
            fileWriter.write(jsonObject.toString().replace("\\\\", "\\"));
        } catch (IOException e) {
            e.printStackTrace();
        }
    ...
    private void unloadFertilizer() {
        ...
        try {
            data.save(new File(Main.plugin.getDataFolder().getParentFile().getParentFile(), world.getName() + File.separator + "draimfarming_data" + File.separator + "fertilizers.yml"));
        }
        catch (IOException e) {
            e.printStackTrace();
            AdventureUtil.consoleMessage("<red>[DraimFarming] Не удалось сохранить данные о горшке для мира " + world.getName() + "</red>");
        }
    }
    ...
    private void unloadSprinkler() {
        ...
        try {
            data.save(new File(Main.plugin.getDataFolder().getParentFile().getParentFile(), world.getName() + File.separator + "draimfarming_data" + File.separator + "sprinklers.yml"));
        }
        catch (IOException e) {
            e.printStackTrace();
            AdventureUtil.consoleMessage("<red>[DraimFarming] Не удалось сохранить данные о разбрызгивателях для мира " + world.getName() + "</red>");
        }
    }
    ...
    private void unloadCrop() {
        ...
        try {
            data.save(new File(Main.plugin.getDataFolder().getParentFile().getParentFile(), world.getName() + File.separator + "draimfarming_data" + File.separator + "crops.yml"));
        }
        catch (IOException e) {
            e.printStackTrace();
            AdventureUtil.consoleMessage("<red>[DraimFarming] Не удалось сохранить данные об урожае для мира  " + world.getName() + "</red>");
        }
    }
    ...
    public void growWire(int cropTime, int sprinklerTime, int dryTime, boolean compensation) {
        ...
        for (SimpleLocation location : cropCache.keySet()) {
            BukkitTask task = bukkitScheduler.runTaskLaterAsynchronously(Main.plugin, () -> {
    ...
    public void growFrame(int cropTime, int sprinklerTime, int dryTime, boolean compensation) {
        ...
        for (SimpleLocation location : cropCache.keySet()) {
            ...
            BukkitTask task1 = bukkitScheduler.runTaskLater(Main.plugin, () -> {
                Location seedLoc = MiscUtils.getLocation(location);
                if (seedLoc == null) return;
                cropMode.loadChunk(seedLoc);
            }, random + dryTime + random);
            BukkitTask task2 = bukkitScheduler.runTaskLater(Main.plugin, () -> {
                Location seedLoc = MiscUtils.getLocation(location);
                if (seedLoc == null) return;
                if (cropMode.growJudge(seedLoc)) {
                    cropCache.remove(location);
    ...
    public YamlConfiguration loadData(String data, String worldName) {
        return ConfigUtil.readData(new File(Main.plugin.getDataFolder().getParentFile().getParentFile(), worldName + File.separator + "draimfarming_data" + File.separator + data +".yml"));
    }
